...
``````{r, echo = FALSE, warning = FALSE, message = FALSE}
# Question: Which month was most popular for each of Victoria Schwab's books in the past full year?
...
popular_df <- Schwab_df %>%
  ...
  mutate(Series = gsub("The unbound : an Archived novel", "The Archived Series", Series)) %>%
  mutate(Series = gsub("Vicious", "The Archived Series", Series)) %>%
  mutate(Series = gsub("ExtraOrdinary", "Villans/ExtraOrdinary Series", Series)) %>%
...
Pop_series <- ggplot(popular_df) +
  ...
  labs(x = "Checkout Month", 
       ...
       title = "Amount of Checkouts Per Month For Each Victoria Schwab Books and Series")
